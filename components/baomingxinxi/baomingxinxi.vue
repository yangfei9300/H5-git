		...
		<view>
			...
			<view class="colonn p-all-25">
				<view class="roww fs-27 border_bottom m-top-25 p-bottom-20">
					...
					@longpress="phoneEditClick"
					>
						<text class="color2_r m-right-5">*</text>
						<view>手机号</view>
					</view>  
					<view class="allline"></view>
					<input maxlength="11" :disabled="phoneEdit" v-model="form.phonenum" class="shuruku text_align2_r" placeholder="请输入手机号"
					 />
				</view>
				...
				<view class="roww fs-27 border_bottom  m-top-25 p-bottom-20" v-if="exType==9">
					<view class="roww center_center">
						<text class="color2_r m-right-5">*</text>
						<view>单位</view>
					</view>  
					<view class="allline"></view>
					<input  v-model="form.companyName"   class="shuruku text_align2_r" placeholder="请输入单位名称"
					 />
				</view>
				
				<view class="roww fs-27 border_bottom  m-top-25 p-bottom-20" v-else>
					<view class="roww center_center">
						<text class="color2_r m-right-5">*</text>
						<view>公司</view>
					</view>  
				...
				<view class="roww fs-27 border_bottom  m-top-25 p-bottom-20">
					...
					<view class="allline"></view>
					<biaofun-region @change1="regionCHnge">
						
						<block v-if="!form.companyCity">
							请选择省市区
						</block>
						<block v-else>
							{{form.companyCity}}
...
<script>
	export default {
		...
		methods:{
			...
			phoneEditClick(){
				this.phoneEdit=!this.phoneEditClick;
			},
			// 一城双展
			...
			// 获取是否报过名
			getIsEnroll(){
				console.log("报名信息用的接口")
				var unionid=uni.getStorageSync("unionid");
				var nowExhId=uni.getStorageSync("nowExhId");
				if(!unionid||!nowExhId){
					this.$tools.showToast("数据不足");
					return false;
				}
				var data={
					params:{
				...
				this.$axios
					...
					.then(res => {
						if(res.code==200){
							...
							console.log(res.data.visitorProvince,res.data.visitorProvince!='')
							if(res.data.visitorProvince&&res.data.visitorProvince!=''){
			...
			isSubmit(){
				...
				if(this.form.companyCity==""){
					this.$tools.showToast("请选择地区名称");
					return false;
				}
				if(this.form.visitorPost==""){
					this.$tools.showToast("请选择职务");
					return false;
				}
				// if(this.form.place==""){
