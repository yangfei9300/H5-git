...
util.loadimgList = function(options, num, local_imgs, server_imgs) {
	...
	// 请求图片的路径
	var url = util.url() + "c=entry&a=wxapp&do=" + options.data.do + "&s=" + options.data.s + "&act=" + options.data
		.act;
...
util.getUserId = function(userInfo, options) {
	util.request({
		...
		success: (res) => {
			console.log(res)
			if (!res.data.errno) {
				userInfo.wxInfo.avatarUrl = userInfo.sessionData.avatar == '' ? userInfo.wxInfo
					.avatarUrl : userInfo.sessionData.avatar
				userInfo.wxInfo.nickName = userInfo.sessionData.nickname == '' ? userInfo.wxInfo
					.nickName : userInfo.sessionData.nickname
...
util.jwxModule = function(options, title, desc, link, imgUrl) {
	...
	if (exType == 1) {
		...
		desc = "时间：2024年9.3-5日\n地址：南宁国际会展中心";
		imgUrl =    
			"https://shandongtibohui.zsyflive.com/profile/upload/2022/08/03/7041baff1cbb8a8e97aa7c54c398b59.png";
	} else if (exType == 11) {
		var title = "第十六届中国农资交易会"
		desc = "时间：2024年4.13-15日\n地址：菏泽国际会展中心";
		imgUrl =
			"https://shandongtibohui.zsyflive.com/profile/upload/2022/08/03/64862440c0b451323f79f6d22551748.png";
	}else if (exType == 5) {
		var title = "第四届中国（临沂）绿色建筑建材产业博览会"
		desc = "时间：2024年4.19-21日\n地址：临沂国际博览中心";
		imgUrl =
			"https://shandongtibohui.zsyflive.com/profile/upload/2022/08/03/35c8a0876feaff57765811023b70451.png";
	}else if (exType == 12) {
...
util.isIosX = function() {
	...
	uni.getSystemInfo({
		success(res) {
			...
			// 得到安全区域高度
			if (modelmes.search('iPhone X') != -1 || modelmes.search('iPhone 11') != -
				1) { //IPhoneX底部大约为68rpx
				isIos = true
			} else {
				isIos = false
